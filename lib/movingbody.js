(function(){
	...
	MovingBody.prototype.drawOrbit = function(ctx, x, y, zoom) {
		var sin = Math.sin(this.argOfPeriapsis),
			...
			edge = this.distance + magnitude([ctx.canvas.width, ctx.canvas.height]) / zoom, // always greater than farthest point visible
	...
	MovingBody.prototype.collide_quietly = function(other_object) {
		this.quiet_collisions.push(other_object);
	}

	MovingBody.prototype.collide_loudly = function(other_body, callback) {
		this.loud_collisions.push([other_object, callback]);
	}

	MovingBody.prototype.check_collisions = function() {
		...
		for (var i = this.quiet_collisions.length - 1; i >= 0; i--) {
			...
			if (magnitude([this.x - other_body.x,
					this.y - other_body.y]) <
					this.radius + other_body.radius) {
				angle = Math.atan2(this.x - other_body.x,
								   this.y - other_body.y)
				this.x = (this.radius + other_body.radius) * Math.cos(angle);
				this.y = (this.radius + other_body.radius) * Math.sin(angle);
		...
		// if collides loudly, call callback passing x, y and angle
		for (var i = this.loud_collisions.length - 1; i >= 0; i--) {
			...
			if (magnitude([this.x - other_body.x,
				...
				angle = Math.atan2(this.x - other_body.x,
								   this.y - other_body.y)
				callback(other_body.radius * Math.cos(angle),
