(function(){
	...
	Game.prototype.draw = function() {
		...
		if (ship.eccentricity < 1) {
			...
			//ctx.moveTo(pointA);
			//ctx.quadraticCurveTo(pointB, pointC);
		}
		ctx.stroke();
		ctx.closePath();
		// periapsis
		ctx.beginPath();
		ctx.arc(x - ship.periapsis * sin * zoom,
				y + ship.periapsis * cos * zoom,
				3, 0, 2 * Math.PI);
		ctx.fill();
		ctx.closePath();
		// vertex of quadratic
		ctx.beginPath();
		ctx.arc(x - (ship.semimajorAxis) * sin * zoom,
				y + (ship.semimajorAxis) * cos * zoom,
				3, 0, 2 * Math.PI);
		ctx.fill();
		ctx.closePath();
